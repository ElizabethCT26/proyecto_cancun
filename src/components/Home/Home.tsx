


function Home() {
    return (
        <>
            <div className="relative w-full h-[35vh] sm:h-[55vh]">
                <div className="absolute inset-0">
                    <img src="https://adventoursmapp.com/wp-content/uploads/2021/06/1.jpg" alt="Agave" className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-black opacity-50" />
                </div>

                <div className="relative w-full h-[35vh] flex items-start px-[6vw] py-[2.5vh]">
                    <div className='flex flex-col sm:gap-[3vw]'>
                        <div className='w-full h-[10vh] '>

                        </div>
                        <div className='text-white w-[75%]'>
                            <p className='text-[2rem] sm:text-[3rem] font-bold'>Cancún</p>
                            <p className="text-[.70rem] sm:text-[1rem] font-light">Descubre el alma de México en Cancún: pasea entre jardines tropicales, disfruta una cata del tequila más emblemático del país y explora espacios dedicados 
                                a la historia y tradición del destilado nacional.</p>
                        </div>
                    </div>
                </div>
            </div>


            <div className=' w-full h-[25vh] flex justify-center flex-col gap-[2vh]'>
                <p className=' h-auto font-bold text-xl w-full text-center'>Selecciona tu próxima aventura</p>
                <div className='flex items-center justify-around w-full sm:w-[70vw] md:w-[60vw] sm:mx-auto'>
                    <div className='flex flex-col justify-center items-center gap-'>
                    <svg
                        className="hover:bg-yellow-100 transition-all duration-300 ease-in-out rounded-full h-[48px] w-[48px] text-black hover:text-yellow-600 stroke-current stroke-[16px]"
                        viewBox="0 0 400 400"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        >
                        <g strokeLinecap="round" strokeLinejoin="round">
                            <path
                            d="M136.321 95.7854C172.731 95.452 196.648 94.9459 208.073 94.2669C225.209 93.2484 263.404 92.4634 270.086 96.3145C276.767 100.166 296 119.797 296 123.105C296 126.414 275.024 145.445 273.677 146.525C272.329 147.605 267.998 149.969 263.344 149.969C258.69 149.969 140.626 153.263 134.497 149.969C128.369 146.674 132.159 115.464 132.159 112.517C132.159 109.571 132.159 106.373 132.159 104.151"
                            fill="none"
                            stroke="currentColor"
                            ></path>
                            <path
                            d="M264.679 172.285C228.269 172.285 204.352 171.945 192.927 171.267C175.791 170.249 137.596 169.464 130.914 173.313C124.233 177.163 105 196.287 105 199.594C105 202.9 125.976 223.423 127.323 224.502C128.671 225.582 133.002 227.945 137.656 227.945C142.31 227.945 260.374 229.739 266.503 226.446C272.631 223.153 268.841 191.955 268.841 189.01C268.841 186.064 268.841 182.868 268.841 180.647"
                            fill="none"
                            stroke="currentColor"
                            ></path>
                            <path
                            d="M206 230C206.156 247.007 206.49 278.674 207 325M207 75C206.494 81.6777 206.16 88.3444 206 95L207 75ZM205.987 153C205.987 157.111 205.996 161.444 206.012 166L205.987 153Z"
                            fill="none"
                            stroke="currentColor"
                            ></path>
                        </g>
                    </svg>


                        <p className='font-light text-[.70rem]'>Vive su magia</p>
                    </div>
                    <div className='flex flex-col justify-center items-center gap-2'>
                        <svg className="hover:bg-yellow-100 transition-all duration-300 ease-in-out rounded-full h-[48px] w-[48px] p-2 hover:fill-yellow-600" fill="#000000" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xmlSpace="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <g> <path d="M230.4,170.667h51.2c4.719,0,8.533-3.823,8.533-8.533c0-4.71-3.814-8.533-8.533-8.533h-51.2 c-4.71,0-8.533,3.823-8.533,8.533C221.867,166.844,225.69,170.667,230.4,170.667z"></path> <path d="M230.4,204.8h51.2c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,204.8,230.4,204.8z"></path> <path d="M503.467,452.267c4.719,0,8.533-3.823,8.533-8.533V384c0-4.71-3.814-8.533-8.533-8.533h-179.2V307.2H460.8v51.2 c0,4.71,3.814,8.533,8.533,8.533s8.533-3.823,8.533-8.533v-59.733c0-4.71-3.814-8.533-8.533-8.533H324.267v-68.267h102.4v51.2 c0,4.71,3.814,8.533,8.533,8.533s8.533-3.823,8.533-8.533v-59.733c0-4.71-3.814-8.533-8.533-8.533H324.267v-68.267h68.267v51.2 c0,4.71,3.814,8.533,8.533,8.533c4.719,0,8.533-3.823,8.533-8.533V128c0-4.71-3.814-8.533-8.533-8.533H290.133V76.8 c0-4.71-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533v42.667H110.933c-4.71,0-8.533,3.823-8.533,8.533v59.733 c0,4.71,3.823,8.533,8.533,8.533s8.533-3.823,8.533-8.533v-51.2h68.267V204.8H76.8c-4.71,0-8.533,3.823-8.533,8.533v59.733 c0,4.71,3.823,8.533,8.533,8.533s8.533-3.823,8.533-8.533v-51.2h102.4v68.267H42.667c-4.71,0-8.533,3.823-8.533,8.533V358.4 c0,4.71,3.823,8.533,8.533,8.533c4.71,0,8.533-3.823,8.533-8.533v-51.2h136.533v68.267H8.533C3.823,375.467,0,379.29,0,384 v59.733c0,4.71,3.823,8.533,8.533,8.533s8.533-3.823,8.533-8.533v-51.2h170.667V460.8H8.533c-4.71,0-8.533,3.823-8.533,8.533 s3.823,8.533,8.533,8.533h494.396c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533H324.267v-68.267h170.667v51.2 C494.933,448.444,498.748,452.267,503.467,452.267z M238.933,85.333h34.133v34.133h-34.133V85.333z M307.2,460.8H204.8V136.533 h102.4V460.8z"></path> <path d="M230.4,238.933h51.2c4.719,0,8.533-3.823,8.533-8.533c0-4.71-3.814-8.533-8.533-8.533h-51.2 c-4.71,0-8.533,3.823-8.533,8.533C221.867,235.11,225.69,238.933,230.4,238.933z"></path> <path d="M145.067,110.933c4.71,0,8.533-3.823,8.533-8.533V51.2h204.8v51.2c0,4.71,3.814,8.533,8.533,8.533 s8.533-3.823,8.533-8.533V42.667c0-4.71-3.814-8.533-8.533-8.533H145.067c-4.71,0-8.533,3.823-8.533,8.533V102.4 C136.533,107.11,140.356,110.933,145.067,110.933z"></path> <path d="M230.4,375.467h51.2c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,375.467,230.4,375.467z"></path> <path d="M230.4,443.733h51.2c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,443.733,230.4,443.733z"></path> <path d="M230.4,409.6h51.2c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,409.6,230.4,409.6z"></path> <path d="M230.4,273.067h51.2c4.719,0,8.533-3.823,8.533-8.533S286.319,256,281.6,256h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,273.067,230.4,273.067z"></path> <path d="M230.4,307.2h51.2c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,307.2,230.4,307.2z"></path> <path d="M230.4,341.333h51.2c4.719,0,8.533-3.823,8.533-8.533s-3.814-8.533-8.533-8.533h-51.2c-4.71,0-8.533,3.823-8.533,8.533 S225.69,341.333,230.4,341.333z"></path> </g> </g> </g> </g></svg>
                        <div className='flex flex-col items-center leading-none'>
                            <p className='font-light text-[.70rem] m-0'>Actividades</p>
                        </div>
                    </div>

                    <div className='flex flex-col justify-center items-center gap-2'>
                        <svg fill="#000000" className="hover:bg-yellow-100 transition-all duration-300 ease-in-out rounded-full h-[48px] w-[48px] p-2 hover:fill-yellow-600" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 476.995 476.995" xmlSpace="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M262.918,239.156l4.744-21.113c0.97-4.311,5.25-7.017,9.56-6.052c4.311,0.969,7.021,5.249,6.052,9.56l-4.744,21.113 c-0.836,3.72-4.139,6.248-7.798,6.248c-0.581,0-1.171-0.063-1.762-0.196C264.659,247.746,261.949,243.466,262.918,239.156z M277.565,172.45c-0.948,4.315,1.782,8.582,6.098,9.529c0.578,0.127,1.155,0.188,1.724,0.188c3.675,0,6.985-2.549,7.806-6.286 l15.097-68.754l74.079,16.266c0.578,0.127,1.155,0.188,1.724,0.188c3.675,0,6.985-2.549,7.806-6.286 c0.948-4.315-1.782-8.582-6.098-9.529L311.72,91.5l11.771-53.606c0.948-4.315-1.782-8.582-6.098-9.529 c-4.315-0.948-8.582,1.782-9.529,6.098l-11.771,53.607l-74.079-16.266c-4.315-0.949-8.582,1.781-9.53,6.098 c-0.947,4.315,1.783,8.582,6.098,9.529l74.079,16.266L277.565,172.45z M268.696,292.89l-4.622,21.05l15.873-3.74 c4.303-1.008,8.608,1.652,9.622,5.952c1.013,4.301-1.652,8.608-5.952,9.622l-23.349,5.501l-10.976,49.987l15.873-3.74 c4.302-1.016,8.608,1.652,9.622,5.952c1.013,4.301-1.652,8.608-5.952,9.622l-23.349,5.501l-5.376,24.482 c-2.962,13.494-10.644,25.161-21.756,33.184l11.704,5.484c4.001,1.874,5.725,6.638,3.85,10.638c-1.36,2.904-4.242,4.608-7.25,4.608 c-1.137,0-2.291-0.243-3.389-0.758l-23.713-11.11c-4.489,1.159-9.108,1.749-13.767,1.749c-4.011,0-8.05-0.436-12.063-1.316 l-30.046-6.598c-25.445-5.587-41.6-30.833-36.013-56.277c1.79-8.151,0.298-16.513-4.2-23.542 c-4.499-7.03-11.465-11.888-19.617-13.678l-25.554-5.611c-4.315-0.947-7.045-5.214-6.098-9.529c0.948-4.315,5.215-7.049,9.53-6.098 l25.554,5.611c12.326,2.706,22.86,10.051,29.662,20.681c6.802,10.63,9.058,23.271,6.351,35.598 c-3.695,16.827,6.989,33.523,23.817,37.219l30.046,6.598c4.506,0.989,9.064,1.188,13.511,0.628l-4.893-20.202 c-1.04-4.294,1.598-8.618,5.892-9.658c4.298-1.042,8.618,1.598,9.659,5.893l4.539,18.742c0.411-0.243,0.822-0.484,1.226-0.743 c8.938-5.72,15.114-14.577,17.39-24.942l5.376-24.483l-18.896-14.776c-3.48-2.722-4.095-7.749-1.374-11.229 c2.722-3.48,7.75-4.097,11.23-1.374l12.846,10.045l10.976-49.989l-18.895-14.776c-3.481-2.722-4.096-7.749-1.374-11.229 c2.721-3.48,7.75-4.096,11.229-1.374l12.846,10.045l4.622-21.049l-68.6-219.432c-1.204-3.851,0.648-7.995,4.32-9.667L319.746,0.72 c3.233-1.474,7.056-0.636,9.376,2.059l93.923,109.02c2.634,3.057,2.579,7.596-0.128,10.588L268.696,292.89z M201.847,71.994 l62.949,201.356l141.518-156.46l-85.396-99.123L201.847,71.994z"></path> </g></svg>
                        <p className='font-light text-[.70rem]'>Festividades</p>
                    </div>
                    <div className='flex flex-col justify-center items-center gap-2'>
                        <svg fill="#000000" className="hover:bg-yellow-100 transition-all duration-300 ease-in-out rounded-full h-[48px] w-[48px] p-2 hover:fill-yellow-600" height="64px" width="64px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xmlSpace="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M151.755,32.206c-4.783,0-8.661,3.878-8.661,8.661v19.312h-14.147V40.867c0-4.783-3.878-8.661-8.661-8.661 c-4.783,0-8.661,3.878-8.661,8.661v55.944c0,4.783,3.878,8.661,8.661,8.661c4.783,0,8.661-3.878,8.661-8.661v-19.31h14.147v19.31 c0,4.783,3.878,8.661,8.661,8.661s8.661-3.878,8.661-8.661V40.867C160.416,36.084,156.538,32.206,151.755,32.206z"></path> </g> </g> <g> <g> <path d="M201.393,32.206c-13.451,0-24.395,10.944-24.395,24.396v24.474c0,13.451,10.944,24.395,24.395,24.395 c13.452,0,24.396-10.944,24.396-24.395V56.602C225.789,43.15,214.845,32.206,201.393,32.206z M208.468,81.076 c0,3.9-3.173,7.073-7.074,7.073c-3.9,0-7.073-3.173-7.073-7.073V56.602c0-3.901,3.173-7.074,7.073-7.074 c3.901,0,7.074,3.173,7.074,7.074V81.076z"></path> </g> </g> <g> <g> <path d="M338.466,88.15h-22.808V77.74h12.796c4.783,0,8.661-3.878,8.661-8.661c0-4.783-3.878-8.661-8.661-8.661h-12.796v-10.89 h22.808c4.783,0,8.661-3.878,8.661-8.661s-3.878-8.661-8.661-8.661h-31.469c-4.783,0-8.661,3.878-8.661,8.661v55.944 c0,4.783,3.878,8.661,8.661,8.661h31.469c4.783,0,8.661-3.878,8.661-8.661S343.249,88.15,338.466,88.15z"></path> </g> </g> <g> <g> <path d="M403.262,88.15h-22.808V40.867c0-4.783-3.878-8.661-8.661-8.661s-8.661,3.878-8.661,8.661v55.944 c0,4.783,3.878,8.661,8.661,8.661h31.469c4.783,0,8.661-3.878,8.661-8.661S408.045,88.15,403.262,88.15z"></path> </g> </g> <g> <g> <path d="M275.064,32.206h-31.469c-4.783,0-8.661,3.878-8.661,8.661s3.878,8.661,8.661,8.661h7.073v47.283 c0,4.783,3.878,8.661,8.661,8.661c4.783,0,8.661-3.878,8.661-8.661V49.528h7.074c4.783,0,8.661-3.878,8.661-8.661 S279.847,32.206,275.064,32.206z"></path> </g> </g> <g> <g> <path d="M484.414,234.103c-4.783,0-8.661,3.878-8.661,8.661v251.914h-80.519v-50.63c0-19.837-16.138-35.975-35.974-35.975 c-19.837,0-35.975,16.138-35.975,35.975v50.63h-31.31v-50.63c0-19.837-16.138-35.975-35.974-35.975 c-19.836,0-35.974,16.138-35.974,35.975v50.63h-31.31v-50.63c0-19.837-16.138-35.975-35.975-35.975 c-19.836,0-35.974,16.138-35.974,35.975v50.63h-80.52v-48.67c0-4.783-3.878-8.661-8.661-8.661c-4.783,0-8.661,3.878-8.661,8.661 v57.331c0,4.783,3.878,8.661,8.661,8.661h456.828c4.783,0,8.661-3.878,8.661-8.661V242.764 C493.075,237.981,489.197,234.103,484.414,234.103z M171.393,494.678h-37.306v-50.63c0-10.286,8.368-18.653,18.652-18.653 c10.286,0,18.653,8.368,18.653,18.653V494.678z M274.653,494.678h-37.306v-50.63c0-10.286,8.368-18.653,18.653-18.653 c10.285,0,18.652,8.368,18.652,18.653V494.678z M377.912,494.678h-37.306v-50.63c0-10.286,8.368-18.653,18.653-18.653 c10.285,0,18.652,8.368,18.652,18.653V494.678z"></path> </g> </g> <g> <g> <path d="M85.389,335.902c-13.371,0-24.251,10.878-24.251,24.251v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C109.639,346.781,98.76,335.902,85.389,335.902z M92.317,384.402H78.46v-24.25 c0-3.82,3.108-6.929,6.929-6.929c3.821,0,6.928,3.108,6.928,6.929V384.402z"></path> </g> </g> <g> <g> <path d="M221.878,335.902c-13.373,0-24.25,10.878-24.25,24.251v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C246.128,346.781,235.25,335.902,221.878,335.902z M228.806,384.402H214.95v-24.25 c0-3.82,3.108-6.929,6.928-6.929s6.929,3.108,6.929,6.929V384.402z"></path> </g> </g> <g> <g> <path d="M153.633,335.902c-13.372,0-24.25,10.878-24.25,24.251v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C177.883,346.781,167.005,335.902,153.633,335.902z M160.561,384.402h-13.856v-24.25 c0-3.82,3.108-6.929,6.928-6.929s6.929,3.108,6.929,6.929V384.402z"></path> </g> </g> <g> <g> <path d="M290.124,335.902c-13.371,0-24.251,10.878-24.251,24.251v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C314.373,346.781,303.495,335.902,290.124,335.902z M297.051,384.402h-13.856v-24.25 c0-3.82,3.108-6.929,6.929-6.929s6.928,3.108,6.928,6.929V384.402z"></path> </g> </g> <g> <g> <path d="M426.612,335.902c-13.372,0-24.25,10.878-24.25,24.251v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C450.863,346.781,439.985,335.902,426.612,335.902z M433.541,384.402h-13.856v-24.25 c0-3.82,3.108-6.929,6.928-6.929c3.82,0,6.929,3.108,6.929,6.929V384.402z"></path> </g> </g> <g> <g> <path d="M358.368,335.902c-13.371,0-24.251,10.878-24.251,24.251v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C382.618,346.781,371.74,335.902,358.368,335.902z M365.296,384.402H351.44v-24.25 c0-3.82,3.108-6.929,6.929-6.929s6.928,3.108,6.928,6.929V384.402z"></path> </g> </g> <g> <g> <path d="M85.389,251.895c-13.371,0-24.251,10.878-24.251,24.25v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C109.639,262.773,98.76,251.895,85.389,251.895z M92.317,300.394H78.46v-24.25 c0-3.82,3.108-6.928,6.929-6.928c3.821,0,6.928,3.108,6.928,6.928V300.394z"></path> </g> </g> <g> <g> <path d="M221.878,251.895c-13.373,0-24.25,10.878-24.25,24.25v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C246.128,262.773,235.25,251.895,221.878,251.895z M228.806,300.394H214.95v-24.25 c0-3.82,3.108-6.928,6.928-6.928s6.929,3.108,6.929,6.928V300.394z"></path> </g> </g> <g> <g> <path d="M153.633,251.895c-13.372,0-24.25,10.878-24.25,24.25v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C177.883,262.773,167.005,251.895,153.633,251.895z M160.561,300.394h-13.856v-24.25 c0-3.82,3.108-6.928,6.928-6.928s6.929,3.108,6.929,6.928V300.394z"></path> </g> </g> <g> <g> <path d="M290.124,251.895c-13.371,0-24.251,10.878-24.251,24.25v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C314.373,262.773,303.495,251.895,290.124,251.895z M297.051,300.394h-13.856v-24.25 c0-3.82,3.108-6.928,6.929-6.928s6.928,3.108,6.928,6.928V300.394z"></path> </g> </g> <g> <g> <path d="M426.612,251.895c-13.372,0-24.25,10.878-24.25,24.25v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C450.863,262.773,439.985,251.895,426.612,251.895z M433.541,300.394h-13.856v-24.25 c0-3.82,3.108-6.928,6.928-6.928c3.82,0,6.929,3.108,6.929,6.928V300.394z"></path> </g> </g> <g> <g> <path d="M358.368,251.895c-13.371,0-24.251,10.878-24.251,24.25v32.91c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.91C382.618,262.773,371.74,251.895,358.368,251.895z M365.296,300.394H351.44v-24.25 c0-3.82,3.108-6.928,6.929-6.928s6.928,3.108,6.928,6.928V300.394z"></path> </g> </g> <g> <g> <path d="M85.388,167.887c-13.371,0-24.251,10.878-24.251,24.251v32.911c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.911C109.637,178.767,98.759,167.887,85.388,167.887z M92.317,216.388H78.46v-24.25 c0-3.82,3.108-6.929,6.929-6.929c3.821,0,6.928,3.108,6.928,6.929V216.388z"></path> </g> </g> <g> <g> <path d="M221.876,167.887c-13.372,0-24.25,10.878-24.25,24.251v32.911c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.911C246.127,178.767,235.249,167.887,221.876,167.887z M228.806,216.388H214.95v-24.25 c0-3.82,3.108-6.929,6.928-6.929s6.929,3.108,6.929,6.929V216.388z"></path> </g> </g> <g> <g> <path d="M153.632,167.887c-13.373,0-24.25,10.878-24.25,24.251v32.911c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.911C177.882,178.767,167.004,167.887,153.632,167.887z M160.561,216.388h-13.856v-24.25 c0-3.82,3.108-6.929,6.928-6.929s6.929,3.108,6.929,6.929V216.388z"></path> </g> </g> <g> <g> <path d="M290.122,167.887c-13.371,0-24.251,10.878-24.251,24.251v32.911c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.911C314.372,178.767,303.494,167.887,290.122,167.887z M297.051,216.388h-13.856v-24.25 c0-3.82,3.108-6.929,6.929-6.929s6.928,3.108,6.928,6.929V216.388z"></path> </g> </g> <g> <g> <path d="M426.612,167.887c-13.372,0-24.25,10.878-24.25,24.251v32.911c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.911C450.863,178.767,439.985,167.887,426.612,167.887z M433.541,216.388h-13.856v-24.25 c0-3.82,3.108-6.929,6.928-6.929c3.82,0,6.929,3.108,6.929,6.929V216.388z"></path> </g> </g> <g> <g> <path d="M358.367,167.887c-13.371,0-24.251,10.878-24.251,24.251v32.911c0,4.783,3.878,8.661,8.661,8.661h31.178 c4.783,0,8.661-3.878,8.661-8.661v-32.911C382.617,178.767,371.739,167.887,358.367,167.887z M365.296,216.388H351.44v-24.25 c0-3.82,3.108-6.929,6.929-6.929s6.928,3.108,6.928,6.929V216.388z"></path> </g> </g> <g> <g> <path d="M499.235,116.665h-38.846V8.661c0-4.783-3.878-8.661-8.661-8.661h-70.049c-4.783,0-8.661,3.878-8.661,8.661 s3.878,8.661,8.661,8.661h61.388v99.344H68.933V17.322h283.876c4.783,0,8.661-3.878,8.661-8.661S357.592,0,352.809,0H60.272 c-4.783,0-8.661,3.878-8.661,8.661v108.004H12.765c-4.783,0-8.661,3.878-8.661,8.661v24.663c0,4.783,3.878,8.661,8.661,8.661h6.16 v258.487c0,4.783,3.878,8.661,8.661,8.661c4.783,0,8.661-3.878,8.661-8.661V158.651h439.507v55.243 c0,4.783,3.878,8.661,8.661,8.661c4.783,0,8.661-3.878,8.661-8.661v-55.243h6.16c4.783,0,8.661-3.878,8.661-8.661v-24.663 C507.896,120.543,504.018,116.665,499.235,116.665z M490.574,141.33H21.426v-7.341h469.148V141.33z"></path> </g> </g> </g></svg>
                        <p className='font-light text-[.70rem]'>Hoteles</p>
                    </div>

                    <div className='flex flex-col justify-center items-center gap-2'>
                        <div className="group w-[48px] h-[48px] p-2 cursor-pointer hover:bg-yellow-100 transition-all duration-300 ease-in-out rounded-full">
                            <svg
                                className="w-full h-full transition-colors duration-300"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                            >
                                <g>
                                <title>Restaurante</title>
                                <path
                                    d="M57.49,47.74,425.92,416.17a37.28,37.28,0,0,1,0,52.72h0a37.29,37.29,0,0,1-52.72,0l-90-91.55A32,32,0,0,1,274,354.91v-5.53a32,32,0,0,0-9.52-22.78l-11.62-10.73a32,32,0,0,0-29.8-7.44h0A48.53,48.53,0,0,1,176.5,295.8L91.07,210.36C40.39,159.68,21.74,83.15,57.49,47.74Z"
                                    className="stroke-black group-hover:stroke-yellow-500 transition-colors"
                                    strokeWidth={32}
                                    strokeLinejoin="round"
                                />
                                <path
                                    d="M400,32l-77.25,77.25A64,64,0,0,0,304,154.51v14.86a16,16,0,0,1-4.69,11.32L288,192"
                                    className="stroke-black group-hover:stroke-yellow-500 transition-colors"
                                    strokeWidth={32}
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                />
                                <path
                                    d="M320,224l11.31-11.31A16,16,0,0,1,342.63,208h14.86a64,64,0,0,0,45.26-18.75L480,112"
                                    className="stroke-black group-hover:stroke-yellow-500 transition-colors"
                                    strokeWidth={32}
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                />
                                <line
                                    x1="440"
                                    y1="72"
                                    x2="360"
                                    y2="152"
                                    className="stroke-black group-hover:stroke-yellow-500 transition-colors"
                                    strokeWidth={32}
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                />
                                <path
                                    d="M200,368,100.28,468.28a40,40,0,0,1-56.56,0h0a40,40,0,0,1,0-56.56L128,328"
                                    className="stroke-black group-hover:stroke-yellow-500 transition-colors"
                                    strokeWidth={32}
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                />
                                </g>
                            </svg>
                            </div>


                        <p className='font-light text-[.70rem]'>Restaurante</p>
                    </div>
                        <div className='flex flex-col justify-center items-center gap-2 group cursor-pointer'>
                        <svg
                            className="transition-all duration-300 ease-in-out rounded-full h-[48px] w-[48px] p-2 hover:bg-yellow-100"
                            viewBox="0 0 24 24"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <g>
                            <path
                                d="M12 16C13.6569 16 15 14.6569 15 13C15 11.3431 13.6569 10 12 10C10.3431 10 9 11.3431 9 13C9 14.6569 10.3431 16 12 16Z"
                                className="stroke-black group-hover:stroke-yellow-600 transition-colors"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                            />
                            <path
                                d="M3 16.8V9.2C3 8.0799 3 7.51984 3.21799 7.09202C3.40973 6.71569 3.71569 6.40973 4.09202 6.21799C4.51984 6 5.0799 6 6.2 6H7.25464C7.37758 6 7.43905 6 7.49576 5.9935C7.79166 5.95961 8.05705 5.79559 8.21969 5.54609C8.25086 5.49827 8.27836 5.44328 8.33333 5.33333C8.44329 5.11342 8.49827 5.00346 8.56062 4.90782C8.8859 4.40882 9.41668 4.08078 10.0085 4.01299C10.1219 4 10.2448 4 10.4907 4H13.5093C13.7552 4 13.8781 4 13.9915 4.01299C14.5833 4.08078 15.1141 4.40882 15.4394 4.90782C15.5017 5.00345 15.5567 5.11345 15.6667 5.33333C15.7216 5.44329 15.7491 5.49827 15.7803 5.54609C15.943 5.79559 16.2083 5.95961 16.5042 5.9935C16.561 6 16.6224 6 16.7454 6H17.8C18.9201 6 19.4802 6 19.908 6.21799C20.2843 6.40973 20.5903 6.71569 20.782 7.09202C21 7.51984 21 8.0799 21 9.2V16.8C21 17.9201 21 18.4802 20.782 18.908C20.5903 19.2843 20.2843 19.5903 19.908 19.782C19.4802 20 18.9201 20 17.8 20H6.2C5.0799 20 4.51984 20 4.09202 19.782C3.71569 19.5903 3.40973 19.2843 3.21799 18.908C3 18.4802 3 17.9201 3 16.8Z"
                                className="stroke-black group-hover:stroke-yellow-600 transition-colors"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                            />
                            </g>
                        </svg>
                        <p className='font-light text-[.70rem]'>Tours</p>
                        </div>

                </div>
            </div>
        </>
    )
}

export default Home
